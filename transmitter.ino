...
#define TX_RATE 40  //Transmit rate in bits per second
...
//defining the pins for the LCD (from the I2C (bus))
...
void setup(){
  ...
  for (int byteC = 0; byteC < strlen(message); byteC++) {
    ...
    for (int bitc = 0; bitc < 8; bitc++) {                         //bitc is bit counter
      ...
      lcd.print(tbit ? "1" : "0");
//      serial.println(tbit ? "1" : "0");                          //debugging
      lcd.setCursor(byteC, 0);
      lcd.cursor();
